...
NumberOfPersonVisiting = st.number_input("Number Of Person Visiting", 1, 10, 3, 1)
NumberOfFollowups = st.number_input("Number Of Follow ups", 1, 10, 4, 1)
ProductPitched = st.selectbox("Product Pitched", ["Basic","Standard","Deluxe","Super Deluxe","King"])
PreferredPropertyStar = st.number_input("Preferred Property Star", 1, 5, 3, 1)
MaritalStatus = st.selectbox("Marital Status", [ "Single","Married","Divorced"])
NumberOfTrips = st.number_input("Number Of Trips", 1, 20, 3, 1)
...
NumberOfChildrenVisiting = st.number_input("Number Of Children Visiting", 0, 10, 1, 1)
Designation = st.selectbox("Designation", ["Executive","Manager","Senior Manager","VP","AVP"])
MonthlyIncome = st.number_input("Monthly Income", 100, 100000, 22000, 100)
